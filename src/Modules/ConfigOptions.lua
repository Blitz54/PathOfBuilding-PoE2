...
Huge sets the radius to 11.
	...
	{ var = "buffQuicksandHourglass", type = "list", label = "Do you have Quicksand Hourglass?", ifFlag = "Condition:CanGainQuicksandHourglass", list = {{val=0,label="None"},{val="AVERAGE",label="Average"},{val="ACTIVE",label="Active"}}, tooltip = "this will enable the Quicksand Hourglass buff itself.", apply = function(val, modList, enemyModList)
		if val == "AVERAGE" then
			modList:NewMod("Condition:QuicksandHourglassAverage", "FLAG", true, "Config", { type = "Condition", var = "Combat" })
		elseif val == "ACTIVE" then
			modList:NewMod("Condition:QuicksandHourglassActive", "FLAG", true, "Config", { type = "Condition", var = "Combat" })
		end
	end },
